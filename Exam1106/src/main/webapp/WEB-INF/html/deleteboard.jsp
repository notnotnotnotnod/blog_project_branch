...
<body>
    ...
    <%for(int i=0;i<list.size();i++){ %>
        ...
        <form>
        <input type="text" name="bcontent" value=<%out.print(list.get(i).getBcontent()); %>/>
        <input class="btn btn-outline-info" type="button" value="글수정" >
        </form>  	
        <form>
        <input id="check_hidden" type="hidden"	name="bno" value=<%out.print(list.get(i).getBno()); %> />
        <input class="btn btn-outline-info" type="button" value="글삭제" style="float: right;"> 
        </form>      
